...
客户原话：「book button 的金色要高端一些，不要咖喱色的金色。」调了很久才想明白 —— **问题不在金色，在白字。**

### 症结是对比度反推出来的亮度

深色 hero 上放金色按钮，直觉是「金底 + 白字」。但白字要满足 AA（4.5:1），就把底色的亮度**顶死在 ~34% L**。而黄橙色相（40–45°）在 34% 亮度上，物理上就是咖喱/芥末色 —— 这不是审美问题，是色彩空间的事实。

在这个约束里怎么调都是死路：

| 试过的 | 结果 |
|---|---|
| 降饱和度去土气 | 34% L 的低饱和黄 = 土褐色，更糟 |
| 提亮到 45% L | 白字掉到 3.1:1，AA 挂了 |
| 加金色渐变 | 渐变底端更暗，对比度按最差点算，等于没提亮 |

**解法是把配色反过来：浅香槟底 + 墨字。**

```css
--gold-btn: 42 48% 74%;   /* 主 CTA 填充 —— 浅香槟，配墨字 */
```

```css
.btn-gold {
  @apply btn text-ink hover:shadow-lift;
  background-color: hsl(var(--gold-btn));
}
.btn-gold:hover { background-color: hsl(42 52% 68%); }  /* 深一档，仍 8.4:1 */
```

墨字落在香槟上实测 **10.1:1**，一次性解决三件事：对比度余量大到可以随便调色相、74% 亮度的黄是香槟不是咖喱、按钮在深色 hero 上成了**全屏最亮的元素**（深色按钮会糊进遮罩里）。用纯色填充不用渐变 —— 边缘更利落，老花眼也看得清。

### ⚠️ 反转配色后，sheen 也必须反转

`.btn` 基类的高光扫光是白色的（`rgba(255,255,255,.42)`）。落到浅色填充上**完全看不见**，动画白跑。改成墨色：

```css
.btn-gold::after {                      /* 覆盖基类的白色 sweep */
  background: linear-gradient(105deg, transparent 15%,
    hsl(var(--ink)/0.06) 45%, hsl(var(--ink)/0.12) 50%,
    hsl(var(--ink)/0.06) 55%, transparent 85%);
}
```

同理，`--gold-deep` 作为**文字色**要单独一个 token，和填充色分开养。我们最后是 `36 64% 34%`（从 42% 压下来），才在 `--paper` 和 `--gold-soft` 徽章底上同时过 AA。**填充金和文字金永远是两个值**，共用一个必然有一边挂。

### 药丸半径：按钮和卡片分开

客户说按钮「显得有点大」时，真实诉求往往是**形状**不是尺寸。全站 `--radius: 2px`（编辑感的硬边卡片）配药丸按钮不冲突 —— 给按钮自己一个 token：

```css
--radius: 2px;         /* 卡片 / 图片框 —— 保持硬边 */
--btn-radius: 999px;   /* 按钮自带,与卡片解耦 */
```
...
药丸让同样 padding 的按钮**视觉上轻一档**，不用真的缩小点击区（保住 44px 触控标准）。

### 手机 hero 的按钮排布

```html
<div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
  <a class="btn-gold text-sm sm:text-base">Book Online</a>
  <a class="btn-ghost text-sm sm:text-base">Call …</a>
</div>
```

- 手机**竖排左对齐**，不要横排：两个按钮挤一行会各自压到 ~46% 宽，标签换行，比竖排更笨重
- `text-sm sm:text-base` —— 手机降一档字号是显瘦最有效的一步（padding 不动）
- `items-start` 让按钮**按内容宽**，不要满宽。满宽按钮在窄屏上是「App 感」，不是「精品店感」
- ⚠️ 但按钮字号降下来后，要回去复查 §2 的 H1 —— 细衬线标题必须**明显大于**按钮文字，这一步很容易把两者拉平
...
- [ ] 主 CTA 是全屏最亮元素，且**不是**「白字 + 34% 亮度黄底」的咖喱配色
- [ ] 填充金与文字金是两个独立 token，各自过 AA
- [ ] 浅色填充按钮的 sheen 已改成墨色（白色 sweep 在浅底上不可见）
...
> **金色按钮土，八成是白字的锅** → 白字把底色顶到 34% 亮度，那个亮度的黄就是咖喱；反过来用香槟底+墨字。
